...
// Curated color palettes - more sophisticated combinations
const palettes = [
    // Sunset Vibes
...
export const AlbumCover: React.FC<AlbumCoverProps> = ({ seed, size = 'md', className = '' }) => {
    ...
    return (
        <div
            className={`${sizeClasses[size]} rounded-md shadow-lg flex-shrink-0 overflow-hidden ${className}`}
            style={coverStyle}
        />
    );
};

// Larger variant with optional icon overlay
interface AlbumCoverLargeProps extends AlbumCoverProps {
    showIcon?: boolean;
    icon?: React.ReactNode;
}

export const AlbumCoverLarge: React.FC<AlbumCoverLargeProps> = ({
    seed,
    size = 'xl',
    className = '',
    showIcon = false,
    ...
    return (
        <div
            className={`${sizeClasses[size]} rounded-lg shadow-xl flex-shrink-0 relative overflow-hidden ${className}`}
            style={coverStyle}
        >
